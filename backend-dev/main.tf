...
resource "aws_kms_key" "terraform_state_key" {
  description             = "KMS key for Dev Terraform state file encryption"
...
resource "aws_s3_bucket" "terraform_state_bucket" {
  bucket = "dev-tf-states-bucket-20250615" # Keep changing this name if the bucket already exists!

  tags = {
    Name        = "Terraform State Bucket Dev" # REMOVED PARENTHESES
    Environment = "Dev"
    Project     = "Portfolio"
  }
}
...
resource "aws_dynamodb_table" "terraform_locks" {
  name         = "dev-tf-locks-20250615" # Keep changing this name if the table already exists!
  hash_key     = "LockID"
  ...
  tags = {
    Name        = "Terraform State Lock Table Dev" # REMOVED PARENTHESES
    Environment = "Dev"
    Project     = "Portfolio"
  }
}
...
output "s3_bucket_name" {
  description = "Name of the S3 bucket for Terraform state in Dev"
  value       = aws_s3_bucket.terraform_state_bucket.bucket
}

output "dynamodb_table_name" {
  description = "Name of the DynamoDB table for Terraform locks in Dev"
  value       = aws_dynamodb_table.terraform_locks.name
}

output "kms_key_arn" {
  description = "ARN of the KMS key used for Terraform state encryption in Dev"
